...
class PreShiftToken(nn.Module):
    ...
    def forward(self, x, cache=None, cache_key=None, **kwargs):
        ...
        if exists(cache):
            ...
            dummy_top, dummy_left = torch.zeros_like(
                dummy_top), torch.zeros_like(dummy_left)
...
class Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        shared_ff_ids=None,
        # use cache-friendly masked attention instead of sparse one
        ...
        for (ind, sparse_attn, attn_type, attn_id, ff_id) in \
            ...
            if attn_type == 'full':
                ...
                if optimize_for_inference:
                    attn_class = partial(
                        Attention, stable=stable, static_mask=self._get_attention_mask(attn_type))
                else:
            ...
            attn, reused_attn_type = shared_attn_layers.get(
                attn_id, (None, None))
            if not exists(attn):
            ...
            if isinstance(attn, Attention) and not isinstance(attn, SparseAttention):
            ...
            if shift_tokens:
                attn = CachedAs(f'preshift_attn_{ind}', PreShiftToken(
                    attn, image_size=image_fmap_size, seq_len=seq_len))
                ff = CachedAs(f'preshift_ff_{ind}', PreShiftToken(
                    ff, image_size=image_fmap_size, seq_len=seq_len))
    ...
    def _get_attention_mask(self, attn_type):
        ...
        if attn_type == 'axial_row':
            ...
            for col in range(self.image_fmap_size):
                begin = text_len + col
                static_mask[begin::self.image_fmap_size,
                            begin::self.image_fmap_size] = True
        else:
            raise ValueError(
